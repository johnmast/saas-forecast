module PlanManager
  ...
  def plan_check (action, subject)
    ...
    elsif current_plan.stripe_id == "free-plan"
      check_free_plan_restrictions(action, subject)
    else
      check_premium_plan_restrictions(action, subject)
    end
  end
  ...
  def check_no_plan_restrictions (action, subject)
    if action == "create"
      if subject == "weather_grid"
        return "You need a plan to create a WeatherGrid. Please visit <a href='/subscriptions'>Plans</a> to buy a plan!"
      end
    end
  end
  ...
  def check_premium_plan_restrictions(action, subject)
    if action == "create"
      if subject == "weather_grid"
        count = WeatherGrid.count
        if count >= 6
